...
%            ksdensity()
% scale:  scale the display of the density curves by this figure. Useful for more crowded plots. (Marina's edit)
%
% OUTPUT
%
% h:     figure handle
...
if isempty(find(strcmp(varargin,'scale')))==0
   scaledensity = varargin{find(strcmp(varargin,'scale'))+1};
else
   scaledensity=1;
end
...
for i=1:size(Y,2)
    ...
    if isempty(b)==0
        [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i), 'Kernel', 'epanechnikov');
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{i}, 'Kernel', 'epanechnikov');
    end
...
for i=i:size(Y,2)
    if isempty(lc) == 1
        if setX == 0
            h(i)=fill([F(:,i)*scaledensity+i;flipud(i-F(:,i)*scaledensity)],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none');
        else
            h(i)=fill([F(:,i)*scaledensity+x(i);flipud(x(i)-F(:,i)*scaledensity)],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none');
        end
    else
        if setX == 0
            h(i)=fill([F(:,i)*scaledensity+i;flipud(i-F(:,i)*scaledensity)],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        else
            h(i)=fill([F(:,i)*scaledensity+x(i);flipud(x(i)-F(:,i)*scaledensity)],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        end
    end
    hold on
    if setX == 0
