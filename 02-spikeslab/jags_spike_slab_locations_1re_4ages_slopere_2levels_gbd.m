...
run ../subroutines/prep_jags_initial_sat_2levels2.m
...
[~, ~, structArray] = matjags( ...
    input, ...                     % Observed data   
    fullfile(pwd, './jags_code/tss_int_1re_4ages_hp_slope_taugamma4_2levels_gbd.jags'), ...    % File that contains model definition
...
% for k = 1:2
% structArray(1,k).(monitor{i})=single(structArray(1,k).(monitor{i}));
% end
% end
...
filename = ['./output/' months{today(2)} sprintf('%02.0f', today(3)) '_4ages_location_hp_slope.mat'];
    
save(filename, 'structArray', 'normalize', 'covar', '-v7.3')

hist([structArray(1,1).omega(:,i), structArray(1,1).betaout1(:,i), structArray(1,1).betaout2(:,i), structArray(1,1).betaout3(:,i),...
    structArray(1,1).betaout4(:,i), structArray(1,1).betaout5(:,i), structArray(1,1).betaout6(:,i),], 100)
...
twolevels2.EECA = cell(4,1);
twolevels2.LAC = cell(4,1);
twolevels2.NAME = cell(4,1);
