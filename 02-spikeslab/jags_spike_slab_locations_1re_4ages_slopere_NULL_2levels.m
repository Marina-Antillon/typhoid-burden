...
monitor = {'alpha', 'beta', 'zeta1', 'zeta2', 'zeta3', 'betaprob', ...
    ...
    'sigma31', 'sigma32', 'sigma33','sigma34', 'omega', ...
...
[~, ~, structArray] = matjags( ...
    input, ...                     % Observed data   
    fullfile(pwd, './jags_code/tss_int_1re_4ages_hp_slope_NULL_2levels.jags'), ...    % File that contains model definition
...
filename = ['./output/' months{today(2)} sprintf('%2.0f', today(3)) '_4ages_location_hp_slope_NULL_2levels.mat'];
...
hist([structArray.omega(:,3), structArray.betaout1(:,3), structArray.betaout2(:,3), structArray.betaout3(:,3)], 100)
...
for i=1:4
    twolevels((1:4)+(i-1)*4, 1) = mean([structArray.omega(:,i), structArray.betaout1(:,i), structArray.betaout2(:,i), structArray.betaout3(:,i)])';
    twolevels((1:4)+(i-1)*4, 2) = prctile([structArray.omega(:,i), structArray.betaout1(:,i), structArray.betaout2(:,i), structArray.betaout3(:,i)], 50)';
    twolevels((1:4)+(i-1)*4, 3) = prctile([structArray.omega(:,i), structArray.betaout1(:,i), structArray.betaout2(:,i), structArray.betaout3(:,i)], 2.5)';
    twolevels((1:4)+(i-1)*4, 4) = prctile([structArray.omega(:,i), structArray.betaout1(:,i), structArray.betaout2(:,i), structArray.betaout3(:,i)], 97.5)';
end
