...
initial_null.beta = [table2array(tmp(strcmp(tmp.Name, 'ages_1'), 2)), table2array(tmp(strcmp(tmp.Name, 'ages_2'), 2)), table2array(tmp(strcmp(tmp.Name, 'ages_4'), 2))] ;
...
initial_null.Bcraw(:,1) = initial_null.mu*ones(input.C, 1); 
initial_null.Bcraw(:,2) = initial_null.beta(1)*ones(input.C, 1);  
initial_null.Bcraw(:,3) = initial_null.beta(2)*ones(input.C, 1); 
initial_null.Bcraw(:,4) = initial_null.beta(3)*ones(input.C, 1); 