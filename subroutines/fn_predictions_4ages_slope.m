function [prelambda, lambda] = fn_predictions_4ages_slope(data, model, nsamples, covars, sample, burn, thin)
...
input.cov = double(table2array(data(:,covars)));
...
%       tmp = mvnrnd([model.mu(j,:); model.beta(j,:)'], [model.sigma1(j,:); model.sigma2(j,:); model.sigma3(j,:); model.sigma4(j,:)]); 
        ...
        par.gamma = model.gamma(j,:)';
        par.gamma_int1 = model.gamma_int1(j,:)';
        par.gamma_int2 = model.gamma_int2(j,:)';
        par.gamma_int3 = model.gamma_int3(j,:)';
%         par.sigma = model.sigma(j,:);
        ...
        prelambda(input.country>0, 1, i) = exp(par.alpha(input.country(input.country>0))' + par.zeta1(input.country(input.country>0))' + input.cov(input.country>0,:)*par.gamma + input.cov(input.country>0,:)*par.gamma_int1);
        prelambda(input.country>0, 2, i) = exp(par.alpha(input.country(input.country>0))' + par.zeta2(input.country(input.country>0))' + input.cov(input.country>0,:)*par.gamma + input.cov(input.country>0,:)*par.gamma_int2);
        prelambda(input.country>0, 3, i) = exp(par.alpha(input.country(input.country>0))' + input.cov(input.country>0,:)*par.gamma);
        prelambda(input.country>0, 4, i) = exp(par.alpha(input.country(input.country>0))' + par.zeta3(input.country(input.country>0))' + input.cov(input.country>0,:)*par.gamma + input.cov(input.country>0,:)*par.gamma_int3);
        ...
        prelambda(input.country==0, 1, i) = exp(par.mu + par.beta(1) + input.cov(input.country==0,:)*par.gamma + input.cov(input.country==0,:)*par.gamma_int1);
        prelambda(input.country==0, 2, i) = exp(par.mu + par.beta(2) + input.cov(input.country==0,:)*par.gamma + input.cov(input.country==0,:)*par.gamma_int2);
        prelambda(input.country==0, 3, i) = exp(par.mu + input.cov(input.country==0,:)*par.gamma);
        prelambda(input.country==0, 4, i) = exp(par.mu + par.beta(3) + input.cov(input.country==0,:)*par.gamma + input.cov(input.country==0,:)*par.gamma_int3);
