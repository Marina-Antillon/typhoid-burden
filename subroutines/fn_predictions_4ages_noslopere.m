function [prelambda, lambda] = fn_predictions_4ages_noslopere(data, model, nsamples, covars, sample, burn, thin)
    
% data must be in wide format.

input.country = double(categorical(data.region));
...
%         par.sigma = model.sigma(j,:);
        ...
        prelambda(input.country>0, 1, i) = exp(par.alpha(input.country(input.country>0))' + par.beta(1) + input.cov(input.country>0,:)*par.gamma + input.cov(input.country>0,:)*par.gamma_int1);
        prelambda(input.country>0, 2, i) = exp(par.alpha(input.country(input.country>0))' + par.beta(2) + input.cov(input.country>0,:)*par.gamma + input.cov(input.country>0,:)*par.gamma_int2);
        prelambda(input.country>0, 3, i) = exp(par.alpha(input.country(input.country>0))' + input.cov(input.country>0,:)*par.gamma);
        prelambda(input.country>0, 4, i) = exp(par.alpha(input.country(input.country>0))' + par.beta(3) + input.cov(input.country>0,:)*par.gamma + input.cov(input.country>0,:)*par.gamma_int3);
